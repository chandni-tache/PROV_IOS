...
try:
    directory = os.environ["TMPDIR"] + '\\Provlog'
    os.stat(directory)
except:
    try:
        os.mkdir(directory)
...
file_handler = logging.FileHandler(os.environ["TMPDIR"] + '\\Provlog\\provLock.log')
file_handler.setLevel(logging.DEBUG)
file_handler.setFormatter(formatter)

appNum_handler = logging.FileHandler(os.environ["TMPDIR"] + '\\Provlog\\ApplicationRuning.log', 'w')
        ...
        if AccessCode.accessCode == AccessCode.eL:
            ...
            if jsonData['status'] == 'success':
                ...
                if self.options.config_file:
                    ...
                    self.file = open(os.environ["TMPDIR"] + '\\Provlog\\config_file.yaml', 'w')
                    self.file.seek(0, 2)
                    yaml.safe_dump(jsonData, self.file)
                    self.file.close()
                    self.file = open(os.environ["TMPDIR"] + '\\Provlog\\config_file.yaml', 'r')
...
class MainWindow(QMainWindow):
    ...
    def mkDirForScreenCapture(self):
        self.dirName = os.environ["TMPDIR"] + '\\Provlog\\MyScreenShot'
...
if __name__ == "__main__":
    ...
    fp = open(os.environ["TMPDIR"] + '\\Provlog\\testForm_prov1', 'w')
    fp.write("python testForm.py -c config_file.yaml")
    fp.close()
    if os.path.isfile(os.path.expanduser(os.environ["TMPDIR"] + '\\Provlog\\testForm_prov1')):
        default_config_file = os.path.expanduser(os.environ["TMPDIR"] + '\\Provlog\\config_file.yaml')
        print(",_hello 11")
